"""Beta calibrator for IndyCar win probability outputs."""
...
class BetaCalibrator:
    """Platt-scaling (logistic) + isotonic regression calibrator."""
    ...
    def fit(self, raw_probs: np.ndarray, y_true: np.ndarray) -> None:
        ...
        self.is_fitted = True
        logger.info("indycar_calibrator_fitted n=%d", len(y_true))
    ...
    # Keep alias so trainer code using .calibrate() also works
    ...
    def save(self, path: str) -> None:
        with open(path, "wb") as f:
            pickle.dump(self, f)
        logger.info("indycar_calibrator_saved path=%s", path)

    @classmethod
    def load(cls, path: str) -> "BetaCalibrator":
        with open(path, "rb") as f:
            obj = pickle.load(f)
        logger.info("indycar_calibrator_loaded path=%s", path)
        return obj
