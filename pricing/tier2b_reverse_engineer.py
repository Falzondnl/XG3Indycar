"""
IndyCar Tier 2B Reverse Engineer (Family F: Plackett-Luce)
============================================================

Closed-form Plackett-Luce inverse for IndyCar race winner / championship
outright markets.

Closed-form algorithm:
  1. Devig Pinnacle outright market -> fair win probs p_1, ..., p_n (sum=1).
  2. Recover PL skill weights: w_i = p_i / p_1.
  3. Podium (top-3), top-5, top-10 via Gumbel-max MC top-K.

Why Family F for IndyCar?
  - 22-33 driver race winner market.
  - Pinnacle race winner / season championship odds are the canonical input.
  - Same PL skills price podium / top-5 / top-10 derivative markets.
...
@dataclass
class IndyCarTier2BResult:
    driver_ids: List[str]
    skills: np.ndarray
    log_skills: np.ndarray
    win_probs: np.ndarray
    market_id: str = ""
    market_type: str = "race_winner"  # race_winner, indycar_championship
    track_type: str = "overall"  # overall, oval, road, street
    n_drivers: int = 0
    overround: float = 0.0
    ...
    def podium_probs(self) -> np.ndarray:
        return self.top_k_probs(3)
...
class IndyCarTier2BReverseEngineer:
    """PL reverse-engineer for IndyCar race + championship outrights."""

    def __init__(self) -> None:
        self._log = logger.bind(service="IndyCarTier2BReverseEngineer")
    ...
    def reverse_engineer(
        ...
        if n < _MIN_FIELD_SIZE:
            raise ValueError(f"IndyCar Tier 2B requires >= {_MIN_FIELD_SIZE}; got {n}")
        if n > _MAX_FIELD_SIZE:
            raise ValueError(f"IndyCar Tier 2B cap is {_MAX_FIELD_SIZE}; got {n}")
...
_DEFAULT_ENGINEER = IndyCarTier2BReverseEngineer()


def get_tier2b_engineer() -> IndyCarTier2BReverseEngineer:
